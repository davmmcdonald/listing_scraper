...
if len(sys.argv) < 3:
    ...
    print("Usage: python3 ls-kratt-commercial.py <url> <output.csv>")
    sys.exit(1)
...
def split_address(address):
    ...
    city_state_zip = format_string(parts[len(parts) - 1])
...
output_folder = "output/kratt-commercial"
...
addresses = soup.select('div.main-wrap .paragraph:contains("Colorado Springs")')
titles = soup.select('div.main-wrap font[size="5"] *')
flyers = soup.select('a.wsite-button.wsite-button-small.wsite-button-normal span.wsite-button-inner:contains("More Info")')
...
with open(csv_path, 'w', newline='') as csv_file:
    ...
    writer.writerow(['Address', 'City, State, Zip', 'Listing Title', 'Listing / Flyer URL'])
    ...
    for i in range(len(addresses)):
        ...
        flyer_url = 'https://www.krattcommercialproperties.com' + flyers[i].parent['href']
